...
class PanMapper(Bank):
    ...
    def compare_archive(self, df_work, df_bank):
        ...
        for index, row in df_bank_cartao.iterrows():
            ...
            if row["Seguro"] == "S":
                linha_cartao['teste_seguro'] = "1,47 | LIQUIDO | 0,00 | NÃO | SEM VIG. INÍCIO | SEM VIG. TÉRMINO"
            ...
            if row["Seguro"] == "S":
                linha_saque['teste_seguro'] = "1,47 | LIQUIDO | 0,00 | NÃO | SEM VIG. INÍCIO | SEM VIG. TÉRMINO"
            dados_final.append(linha_saque)

        df_bank_cartao = pd.DataFrame(dados_final)

        df_bank_cartao.to_excel("df_bank_cartao.xlsx", index=False)
        ...
        df_result = pd.merge(
            df_bank_cartao,
            df_work_cartao,
            left_on=["Empregador", "Prazo", "Operação"],
            right_on=["Produto", "Parc. Atual", "Operação"],
            how="outer",
            indicator=True
        )
        ...
        print(len(df_open))
        print(len(list_of_close_tables))
