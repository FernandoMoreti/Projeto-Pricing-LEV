...
from ..config.bank.MeucashCardVariables import family_product, group_convenio, operation
from ..config.citys_uf import citys, citys_uf, states
from ..config.grade import grade

class MeucashCardMapper(Bank):

    def read_archive(self, file):
        df = pd.read_excel(io.BytesIO(file), sheet_name=None, header=None)
        return df

    def joinDfs(self, df_bank):

        dfs_limpos = []

        for aba in df_bank:
            df_atual = df_bank[aba]
        
            linha_cabecalho = None
            for idx, row in df_atual.iterrows():
                if row.astype(str).str.strip().str.upper().isin(['CÓDIGO', 'CODIGO']).any():
                    linha_cabecalho = idx
                    break
                
            df_atual.columns = df_atual.iloc[linha_cabecalho].str.strip()
            df_atual.columns = df_atual.columns.str.upper()
            df_atual = df_atual.iloc[linha_cabecalho + 1:].copy()
            
            coluna_codigo = next((col for col in df_atual.columns if str(col).upper() in ['CÓDIGO', 'CODIGO']), None)
            
            if coluna_codigo:
                df_atual['Aba Origem'] = aba
                
                df_atual = df_atual.dropna(subset=[coluna_codigo])
                df_atual = df_atual[df_atual[coluna_codigo].astype(str).str.upper() != 'CÓDIGO']
                df_atual = df_atual[df_atual[coluna_codigo].astype(str).str.upper() != 'CODIGO']
                
                dfs_limpos.append(df_atual)
        
        df_bank = pd.concat(dfs_limpos, ignore_index=True)

        return df_bank

    def compare_archive(self, df_work, df_bank):

        df_bank = self.joinDfs(df_bank)

        list_to_add = []

        for index, row in df_bank.iterrows():

            row["product"] = row["EMPREGADOR"] + " - " +  row["TIPO TAXA"]

            if pd.notna(row["6X"]) and row["6X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "6-6"
                list_to_add.append(rowcopy)
            if pd.notna(row["12X"]) and row["12X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "12-12"
                list_to_add.append(rowcopy)
            if pd.notna(row["24X"]) and row["24X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "24-24"
                list_to_add.append(rowcopy)
            if pd.notna(row["36X"]) and row["36X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "36-36"
                list_to_add.append(rowcopy)
            if pd.notna(row["48X"]) and row["48X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "48-48"
                list_to_add.append(rowcopy)
            if pd.notna(row["60X"]) and row["60X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "60-60"
                list_to_add.append(rowcopy)
            if pd.notna(row["72X"]) and row["72X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "72-72"
                list_to_add.append(rowcopy)
            if pd.notna(row["80X"]) and row["80X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "80-80"
                list_to_add.append(rowcopy)
            if pd.notna(row["96X"]) and row["96X"] != "Ñ aplica":
                rowcopy = row.copy()
                rowcopy["Prazo"] = "96-96"
                list_to_add.append(rowcopy)

        df_bank = pd.DataFrame(list_to_add)
        ...
        for index, row in df_matches.iterrows():
            ...
            value = row["Prazo"].split("-")[0] + "X"

            percent = round(convertValues(row[value] * 100), 2)
    ...
    def create_open_tables(self, list_of_open_tables, model):
        ...
        for row in list_of_open_tables:
            ...
            if "SAQUE" in tableName:
                operation = "SAQUE COMPL."
            elif "REFIN" in tableName or "AUMENTO DE MARGEM" in tableName:
                operation = "REFIN"
            else:
                operation = "CARTÃO"
    ...
    def create_close_tables(self, list_of_close_tables):
        ...
        df = df.drop(['CÓDIGO', 'EMPREGADOR', 'UF', 'TIPO TAXA', 'TAXA', '6X', '12X', '24X',
            '36X', '48X', '60X', '72X', '80X', '96X', 'Aba Origem', 'product',
            'Prazo', '_merge'], axis=1, errors='ignore')
    ...
    def create_close_open_tables(self, list_of_close_open):
        ...
        colunas_remover = ['CÓDIGO', 'EMPREGADOR', 'UF', 'TIPO TAXA', 'TAXA', '6X', '12X', '24X',
            '36X', '48X', '60X', '72X', '80X', '96X', 'Aba Origem', 'product',
            'Prazo', '_merge']
